...
#from CNN_TAM import TAM_CNN
from feature_selection import SD_CAFF
...
# setup_seed(0)
# setup_seed(999999)
def train(dataset, feature_num, feature_type, data_num, root_dir, para):
    setup_seed(987654)
    # print(os.path.join(root_dir, 'd/a.pkl'))
    # # logger setting
    ...
    if dataset == 'IEMOCAP':
        ...
        file = r'../data/MSP_data/MSP_leave_{}_data_16000.pkl'.format(str(data_num + 1))
    elif dataset == 'MELD':
    ...
    sd_caff = SD_CAFF(feature_num=feature_num)

    if torch.cuda.is_available():
        sd_caff = sd_caff.cuda()
    ...
    params = list(sd_caff.parameters()) + list(center_loss.parameters()) + list(center_loss_sex.parameters())
    ...
    for i in range(Epoch):
        ...
        for _, data in tqdm(enumerate(train_loader)):
            ...
            if feature_type == "mfcc":
                ...
                x = x3
            elif feature_type == 'd_dd':
                p = para[0]
                x = torch.cat((p[0] * x2, p[1] * x3), dim=-2)
            elif feature_type == "mfcc_d":
                p = para[1]
                x = torch.cat((p[0] * x1,  p[1] * x2), dim=-2)
            elif feature_type == "mfcc_dd":
                p = para[2]
                x =torch.cat((p[0] * x1, p[1] * x3), dim=-2)
            ...
            attn, out_emotion, out_gender, out_emotion_center, out_gender_center = sd_caff(x1,x)
            #
        ...
        for i in range(len(val_y)):
            ...
            out_a, out_emotion, out_gender, out_emotion_center, out_gender_center = sd_caff(x1.unsqueeze(0),
                                                                                              x.unsqueeze(0))
            #
    ...
    result_file = os.path.join(res_dir, '{}/MSP_{}_results.pkl'.format(feature_type, data_num))
...
if __name__ == '__main__':
    ...
    datasets = ['IEMOCAP']
    feature_nums = [2,2]
    #feature_types = [ 'mfcc_dd', 'd_dd']
    feature_types = ['mfcc_d_dd']
    num = 5
    ...
    for dataset in datasets:
        for feature_type in feature_types:
            feature_num = feature_nums[feature_types.index(feature_type)]
            print(feature_num)
            print('start**********************************'+feature_type+'********************************')
            root_dir = "/home/results_feature_selection/"
            avg_WA, avg_UA = 0, 0
            para = [[1, 1], [1, 1], [1, 1], [1, 1, 1]]
            for n in range(num):
                maxWA, maxUA, best_matrix = train(dataset, feature_num, feature_type, n, root_dir, para)
                avg_WA += maxWA
                avg_UA += maxUA
            avg_WA /= num
            avg_UA /= num
            ...
            result_file = os.path.join(result_dir, '{}/avg_results.pkl'.format(feature_type))
